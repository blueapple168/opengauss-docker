...
# used to create initial opengauss directories and if run as root, ensure ownership to the gaussdb user
docker_create_db_directories() {
	...
	if [ "$user" = '0' ]; then
		find "$PGDATA" \! -user gaussdb -exec chown gaussdb '{}' +
		find /var/run/opengauss \! -user gaussdb -exec chown gaussdb '{}' +
	fi
}
	...
	if [ -z "$GS_PASSWORD" ] && [ 'trust' != "$GS_HOST_AUTH_METHOD" ]; then
		...
		cat >&2 <<-'EOE'
			Error: Database is uninitialized and superuser password is not specified.
			       ...
			       connections without a password. This is *not* recgaussdbended.
		...
		cat >&2 <<-'EOWARN'
			********************************************************************************
			WARNING: GS_HOST_AUTH_METHOD has been set to "trust". This will allow
			         ...
			         It is not recgaussdbended to use GS_HOST_AUTH_METHOD=trust. Replace
